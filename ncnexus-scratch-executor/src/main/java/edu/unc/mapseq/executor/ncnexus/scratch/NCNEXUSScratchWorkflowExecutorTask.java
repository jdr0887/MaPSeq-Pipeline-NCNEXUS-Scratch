...
import edu.unc.mapseq.workflow.ncnexus.scratch.NCNEXUSScratchWorkflow;

public class NCNEXUSScratchWorkflowExecutorTask extends TimerTask {

    private final Logger logger = LoggerFactory.getLogger(NCNEXUSScratchWorkflowExecutorTask.class);

    private final WorkflowTPE threadPoolExecutor = new WorkflowTPE();

    private WorkflowBeanService workflowBeanService;

    public NCNEXUSScratchWorkflowExecutorTask() {
        super();
    }
    ...
    public void run() {
        ...
        try {
            List<Workflow> workflowList = workflowDAO.findByName("NCNEXUSScratch");
            if (workflowList == null || (workflowList != null && workflowList.isEmpty())) {
                logger.error("No Workflow Found: {}", "NCNEXUSScratch");
                return;
            }
            ...
            if (attempts != null && !attempts.isEmpty()) {
                ...
                for (WorkflowRunAttempt attempt : attempts) {
                    ...
                    NCNEXUSScratchWorkflow scratchWorkflow = new NCNEXUSScratchWorkflow();
                    attempt.setVersion(scratchWorkflow.getVersion());
                    attempt.setDequeued(new Date());
                    workflowRunAttemptDAO.save(attempt);

                    scratchWorkflow.setWorkflowBeanService(workflowBeanService);
                    scratchWorkflow.setWorkflowRunAttempt(attempt);
                    threadPoolExecutor.submit(new WorkflowExecutor(scratchWorkflow));
